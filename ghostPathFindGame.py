...
from UCS_Ghost import UCS_Ghost
from greedy_Ghost import Greedy_Ghost
from a_Star_Ghost import A_Star_Ghost
...
class GhostPathFindGame(object):
    def __init__(self):
        ...
        self.UCS_Goal = [9,11]
        self.greedyOneGoal = [9,11]
        self.greedyTwoGoal = [9,11]
        self.A_StarGoal = [9,11]
        self.specificGhosts = []
        self.goalList = [self.UCS_Goal, self.greedyOneGoal, self.A_StarGoal, self.greedyTwoGoal]
        ...
        for i,row in enumerate(enviroment()):
            for j,item in enumerate(row):
                ...
                if(item != 0 and item != 3):
                    self.blocksList[item].add(Block(j*32+8,i*32+8,BLACK,16,16))
                # if item == 1:
        ...
        self.ghosts.add(UCS_Ghost(288,256,0,-2, "blinky.png"))
        self.ghosts.add(Greedy_Ghost(288,288,0,-2, "pinky.png"))
        self.ghosts.add(A_Star_Ghost(288,320,0,2, "inky.png"))
        self.ghosts.add(Greedy_Ghost(352,288,0,-2, "clyde.png"))
        
        
        for ghost in self.ghosts:
            self.specificGhosts.append(ghost)
        self.addFood()
    ...
    def runLogic(self):
        if not self.gameOver:
            ...
            count = 0
            for ghost in self.ghosts:
                ghost.update(self.player.newTile, self.goalList[count][0], self.goalList[count][1])
                count += 1
    ...
    def getGoals(self):
        if not self.player.newTile:
            return
        currentX = self.player.rect.x//32
        currentY = self.player.rect.y//32

        self.A_StarGoal = (currentX, currentY)
        ...
        if self.player.currentDirection == "right":
            ...
            for i in range(4):
                if currentX+(i+1) >= len(toSearch):
                    self.greedyOneGoal = (0,currentY)
                else:
                    self.greedyOneGoal = (currentX + i+1, currentY)
                    if toSearch[currentX + i+1] == 0:
                        self.greedyOneGoal = (currentX + i, currentY)
                        break
        elif self.player.currentDirection == "left":
            toSearch = self.grid[currentY]
            for i in range(4):
                if currentX-(i+1) < 0:
                    self.greedyOneGoal = (len(toSearch)-1, currentY)
                else:
                    self.greedyOneGoal = (currentX - (i+1), currentY)
                    if toSearch[currentX - (i+1)] == 0:
                        self.greedyOneGoal = (currentX - i, currentY)
            ...
            for i in range(4):
                self.greedyOneGoal = (currentX, currentY - (i+1))
                if self.grid[currentY-i][currentX] == 0:
                    self.greedyOneGoal = (currentX, currentY - i)
                    break
        ...
        if self.player.currentDirection == "left":
            ...
            for i in range(4):
                if currentX+(i+1) >= len(toSearch):
                    ...
                    self.greedyTwoGoal = (currentX + i+1, currentY)
                    if toSearch[currentX + i+1] == 0:
                        self.greedyTwoGoal = (currentX + i, currentY)
                        break
            ...
            for i in range(4):
                self.greedyTwoGoal = (currentX, currentY - (i+1))
                if self.grid[currentY-i][currentX] == 0:
                    self.greedyTwoGoal = (currentX, currentY - i)
                    break
        elif self.player.currentDirection == "up":
            for i in range(4):
                self.greedyTwoGoal = (currentX, currentY + (i+1))
        ...
        if self.player.currentDirection == "left":
            if currentX <= 1:
                ...
                if not self.grid[currentY][currentX-1] == 0:
                    self.UCS_Goal = (currentX-1, currentY)
                    if not self.grid[currentY][currentX-2] == 0:
                        self.UCS_Goal = (currentX-2, currentY)
                        if not self.grid[currentY+1][currentX-2] == 0:
                            self.UCS_Goal = (currentX-2, currentY+1)
                            if not self.grid[currentY+2][currentX-2] == 0:
                                self.UCS_Goal = (currentX-2, currentY+2)
            ...
            if not self.grid[currentY+1][currentX] == 0:
                self.UCS_Goal = (currentX, currentY+1)
                if not self.grid[currentY+2][currentX] == 0:
                    self.UCS_Goal = (currentX, currentY+2)
                    if not self.grid[currentY+2][currentX-1] == 0:
                        self.UCS_Goal = (currentX-1, currentY+2)
                        if not self.grid[currentY+2][currentX-2] == 0:
                            self.UCS_Goal = (currentX-2, currentY+2)
        ...
        # if self.UCS_Goal == (19,9) or self.UCS_Goal == (-1,9) or self.greedyOneGoal == (19,9) or self.greedyOneGoal == (-1,9) or self.greedyTwoGoal == (19,9) or self.greedyTwoGoal == (-1,9):
        #     print("error")
